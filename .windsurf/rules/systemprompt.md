...
### Padrão de escrita dos dossiês (Fase 4)

- O dossiê deve ser legível para board: use citações curtas (trechos) + “Leitura” (1–2 linhas: o que prova / por que importa) no corpo do texto.
- Evite linguagem de inventário no corpo do dossiê (ex.: “blob”, “OCR2”, “field_id”). Se necessário, deixe termos técnicos apenas no apêndice.
- Em `Descrição`: escreva com substância em prosa. Explique como a intervenção funciona na prática, de ponta a ponta, com regra do mecanismo, governança e cadência quando aplicável. Bullets podem existir, mas não podem ser a única coisa.
- Não crie seção de “Comunicação” como padrão no dossiê; mantenha o foco no mecanismo e na governança.
- No corpo do dossiê, evite parênteses; prefira frases diretas.
- Mantenha rastreabilidade completa em um apêndice no final (path + linhas/IDs), para auditoria. No apêndice, parênteses são aceitáveis quando fizerem parte do nome do arquivo ou do trecho citado.
- Em “Lacunas”, escreva apenas lacunas de viabilidade/execução (pré-condições, decisões pendentes, métricas e critério de sucesso do piloto) — não lacunas de arquivo.
...
## Convenção operacional — escopo do banco

- **Banco/formulário (Postgres export)**: usar `unit_slug=general` como fonte de verdade atual.
- Recorte por unidades (`cajamar`, `gru-food`, `gru-fm`) foi um experimento anterior e é considerado **legado**; só usar quando explicitamente solicitado.
...
Notas operacionais (inventário):

- `evidencias/indice/atualizacao-inventario.report.{md,json}` são **gerados automaticamente** (não editar manualmente).
- Tabelas do formulário (`field_type=table_cell`) são reconstruídas automaticamente em `evidencias/indice/tabelas/<submission_id>/table_*.{md,json}`.
